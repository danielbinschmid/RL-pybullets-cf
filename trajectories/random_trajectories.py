...
class ControlPointSampler:
    ...
    def sample_nxt_wp(
        self, wp: np.ndarray, current_direction: np.ndarray
...
def sample_random_ctrl_points(
    start_wp: np.ndarray,
    std_dev_deg: float = 90,
    n_ctrl_points: int = 10,
    distance_between_ctrl_point: float = 1,
    ...
    wp_sampler = ControlPointSampler(
        std_dev_deg=std_dev_deg, distance=distance_between_ctrl_point
    )
    wps = [Waypoint(coordinate=start_wp, timestamp=0)]
    for i in range(n_ctrl_points):
        new_wp_coordinate, new_dir = wp_sampler.sample_nxt_wp(
            wp=cur_wp, current_direction=cur_dir
        )
        new_wp = Waypoint(coordinate=new_wp_coordinate, timestamp=i + 1)
        wps.append(new_wp)
...
if __name__ == "__main__":
    ...
    sampler = ControlPointSampler(std_dev_deg=90, distance=1)
