...
class MPCCasADiControl(BaseControl):
    ...
    def __init__(self,
        ...
        P = MX.sym('P', Nx + Nx + Nx)  # parameters (which include the initial state and the reference state)
        ...
        NEXT_STATE_WEIGHTING = 0.2

        for k in range(Nhoriz - 1):
            st_ref = P[Nx:2 * Nx]
            st_ref_next = P[2 * Nx:3 * Nx]
            st = X[:, k]
            cont = U[:, k]
            cont_ref = u_ref
            J += (st - st_ref).T @ Q @ (st - st_ref) + (cont - cont_ref).T @ R @ (cont - cont_ref) + \
                 NEXT_STATE_WEIGHTING * ((st - st_ref_next).T @ Q @ (st - st_ref_next))
            st_next = X[:, k + 1]
            k1 = f(st, cont)
            k2 = f(st + h / 2 * k1, cont)
            k3 = f(st + h / 2 * k2, cont)
    ...
    def computeControl(self,
        ...
        self.args['p'] = vertcat(
            ...
            self.state_target_next  # next target state
        )
    ...
    def computeControlfrom2States(self,
                                  control_timestep,
                                  cur_pos,
                                  cur_quat,
                                  cur_vel,
        ...
        cur_ang_vel : ndarray
            (3,1)-shaped array of floats containing the current angular velocity.
        target_pos : ndarray
            (6,1)-shaped array of floats containing the two next desired positions from the trajectory.
        target_rpy : ndarray, optional
            (3,1)-shaped array of floats containing the desired orientation as roll, pitch, yaw.
        target_vel : ndarray, optional
            (3,1)-shaped array of floats containing the desired velocity.
        ...
        # Set position targets for the next target position
        x_pos_target_next = target_pos[3]
        y_target_next = target_pos[4]
        z_target_next = target_pos[5]
        ...
        self.state_target_next = DM(
            [x_pos_target_next, y_target_next, z_target_next, phi_target, theta_target, psi_target, x_pos_dot_target,
             y_dot_target,
             z_dot_target, phi_dot_target, theta_dot_target, psi_dot_target])  # next target state
