...
class Rewards:
    ...
    def get_travelled_distance(self, position: np.ndarray):
        ...
        overall_distance_travelled = np.sum(
            self.distances[:closest_point_idx]
    ...
    def compute_reward(
        self, drone_state: np.ndarray, reached_distance: np.ndarray, bodyrates=None
    ):
        """
        ...
        closest_waypoint_distance, closest_waypoint = self.get_closest_waypoint(
            position
        )
        projection_distance = np.linalg.norm(self.current_projection - position)

        r_t = (
            -10 if (abs(drone_state[7]) > 0.4 or abs(drone_state[8]) > 0.4) else 0
        )  # when its tilted
        ...
        if (
            closest_waypoint_distance <= self.dist_tol
            and not self.wp_rewards[closest_waypoint]
        ):
            self.wp_rewards[closest_waypoint] = 1
            r_wp = np.exp(-closest_waypoint_distance / self.dist_tol)
            self.cur_wp_idx = (
                self.cur_wp_idx + 1
                if self.cur_wp_idx < len(self.trajectory) - 2
                else len(self.trajectory) - 2
            )
        else:
            r_wp = 0
        ...
        s_gd = (
            np.exp(2 * (self.max_reward_distance - projection_distance))
            if projection_distance > self.max_reward_distance
            else 1
        )
        ...
        if (
            np.all(self.trajectory[closest_waypoint] == self.trajectory[-1])
        ) and closest_waypoint_distance < 0.05:
            r += 10
        return r
