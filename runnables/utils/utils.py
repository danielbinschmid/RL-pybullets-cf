...
def compute_metrics(
    all_visisted_positions: np.ndarray,
    successes,
    tracks: List[DiscretizedTrajectory],
    n_discr_level=int(1e4),
):
    ...
    for idx, success in enumerate(tqdm(successes)):
        ...
        if success:
            ...
            high_discr_ref_traj = TrajectoryFactory.get_pol_discretized_trajectory(
                t_waypoints=track, n_points_discretization_level=n_discr_level
            )
            ...
            norms: np.ndarray = np.linalg.norm(
                visited_positions[:, np.newaxis, :] - ref_wps[np.newaxis, :, :], axis=2
            )
...
def compute_metrics_single(
    visited_positions: np.ndarray, track: DiscretizedTrajectory, n_discr_level=int(1e4)
):
