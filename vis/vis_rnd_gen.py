...
def visualize_cone_with_vector(original_vector, random_vector, cone_angle_deg):
    ...
    ax.quiver(0, 0, 0, original_vector[0], original_vector[1], original_vector[2], color='blue', label='Original Vector')
    
    # Plot the random vector
    ax.quiver(0, 0, 0, random_vector[0], random_vector[1], random_vector[2], color='red', label='Random Vector')
...
def generate_and_transform_cone_points(cone_length, cone_angle_deg, direction, position):
    """
    ...
    return translated_points[0].reshape(u.shape), translated_points[1].reshape(u.shape), translated_points[2].reshape(u.shape)

def generate_and_transform_cone_for_vector(base_position, direction, cone_length, cone_angle_deg):
    """
...
def generate_and_transform_cone_at_vector_start(start_position, direction, cone_length, cone_angle_deg):
    """
...
def visualise_vector_cones(positions, cone_angle_deg, traj):
    ...
    # draw initial arrow
    ax.quiver(startpos[0], startpos[1], startpos[2], 
            firstdir[0], firstdir[1], firstdir[2], 
            color='black', length=norm(firstdir), arrow_length_ratio=0.1)
    ...
    for i in range(1, len(positions) - 1):
        ...
        cone_length = 0.25 * vector_magnitude  # Make the cone's length proportional to the vector's magnitude

        # Correcting cone generation to start at the vector's start
        cone_dir = cur_pos - positions[i - 1]
        x_transformed, y_transformed, z_transformed = generate_and_transform_cone_at_vector_start(
        ...
        # Plotting the cone
        ax.plot_surface(x_transformed, y_transformed, z_transformed, color='cyan', alpha=0.5)

        # Plotting the vector extending from the cone's tip
        ax.quiver(cur_pos[0], cur_pos[1], cur_pos[2], 
                cur_dir[0], cur_dir[1], cur_dir[2], 
                color='black', length=vector_magnitude, arrow_length_ratio=0.1)
...
def vis_polynomial(positions, cone_angle_deg, traj):
    ...
    ax.scatter(reference_x, reference_y, reference_z, color="black", marker='x', s=100, label='Target waypoints')
    ...
    legend_handles = [
        mlines.Line2D([], [], color=cmaps[0]([0.5]), marker='o', linestyle='None', markersize=5, label="Generated polynomial trajectory"),
...
def dummyvis():
    ...
    original_vector = np.array([0, 0, 1])  # Assuming this is already aligned with z-axis for simplicity
    cone_angle_deg = 30  # Cone angle in degrees
    std_deviation = np.radians(5)  # Standard deviation in radians for the angle within the cone

    # Generate a random vector
    random_vector = generate_random_vector_in_cone(original_vector, cone_angle_deg, std_deviation)
